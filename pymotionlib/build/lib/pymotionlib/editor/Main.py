...
from pymotionlib.editor.View import ViewWidget
from pymotionlib.editor.ClipMgr import ModifiedClipManager
from pymotionlib.editor.Timeline import TimelineWidget
from pymotionlib.MotionData import MotionData
from pymotionlib.BVHLoader import save 
...
class MainWindow(QMainWindow):
    ...
    def _save_hlp(self, fn: str, fps = None):
        motion = self.clipMgr.clip_list[0].clip.motion
        save(data=motion, fn=fn)
    ...
    def save(self):
        ...
        fn = QFileDialog.getSaveFileName(self, 'Load Motion File',
            filter='Motion files (*.bvh);;All files (*.*)')
        if fn[0] == '':
            return
        self._save_hlp(fn[0], 60)
    ...
    def reCalTimeLine(self):
        num = 0
        for data in self.clipMgr.clip_list:
            num = data.num_frames if data.num_frames > num else num
        self.timeline.frameSlider.setMaximum(num)
    
    def cut(self):
        self.statusBar().showMessage("Invoked <b>Edit|Cut</b>")
        cur_time = self.timeline.frameSlider.value()
        self.clipMgr.clip_list[0].clip.cut(begin = 0, end = cur_time)
        self.reCalTimeLine()
        
    def cutPrevious(self):
        self.statusBar().showMessage("Invoked <b>Edit|Cut</b>")
        cur_time = self.timeline.frameSlider.value()
        self.clipMgr.clip_list[0].clip.cut(begin = cur_time)
        self.reCalTimeLine()
    ...
    def enable_grid(self):
        self.view_widget.use_grid = not self.view_widget.use_grid
    ...
    def createActions(self):
        ...
        self.cutPreviousAct = QAction("Cu&t", self, 
                statusTip="Cut the current selection's contents to the clipboard",
                triggered=self.cutPrevious)
        ...
        self.enableGridAct = QAction("Enable/Disable Grid", self, statusTip="", triggered=self.enable_grid)
    ...
    def createMenus(self):
        ...
        self.fileMenu.addAction(self.enableGridAct)
        ...
        self.editMenu.addAction(self.cutPreviousAct)
